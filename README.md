# Hi 👋 I'm Yashwanth G

⚡ **Software Engineer | ECE Graduate | Backend & Cloud Developer**

## 📈 GitHub Summary

![Profile Details](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Yashwanthg07)



I’m a motivated software engineer with hands-on experience in the **fintech domain**, building and maintaining **scalable backend systems**. I enjoy working on secure, high-performance applications and continuously improving my skills through real-world experience.

---

### 👨‍💻 About Me

* 🎓 ECE Graduate with strong software engineering fundamentals
* 🏦 Engineering Intern at **M2P Fintech**, working on Access Control Server (ACS) systems
* 🧩 Experienced in backend development using **Java, Python, and SQL**
* ☁️ Hands-on with **Google Cloud Platform (GCP)**
* 📊 Skilled in **API testing**, **monitoring**, and Agile development
* 🚀 Focused on writing clean, reliable, and production-ready code

---

### 🛠️ Tech Stack

* **Languages:** Java, Python, SQL
* **Backend:** REST APIs, Database Design, Transaction Processing
* **Cloud & DevOps:** Google Cloud Platform (GCP), Grafana, Postman
* **Tools:** Git, GitHub, VS Code
* **Concepts:** OOP, RDBMS, Agile/Scrum

---

### 💼 Experience Highlight

**Engineering Intern – M2P Fintech**

* Developed backend features for high-traffic fintech security systems
* Improved system reliability using monitoring dashboards (Grafana)
* Performed API testing and debugging to enhance application stability
* Collaborated with cross-functional teams in an Agile environment

---

### 📫 Connect With Me

* 💼 [LinkedIn](https://www.linkedin.com/in/yashwanth-g20/)
* 🌐 [Portfolio](https://yashwanth-portfolio-site.vercel.app/)
* 📧 Email: [yashwanthg07@gmail.com](mailto:yashwanthg07@gmail.com)

⭐ *Thanks for visiting my profile!*
